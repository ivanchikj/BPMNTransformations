...
public class Reverse1 {
	...
	//that means it has only one outgoingflow and that has more than one incoming flow.
	...
	for (Element parallel : candidates) {
	    ...
	    //the number varies depending on the number of incomingFlows and on the aggregateBy parameter
	    //we should divide the number of incomingFlows by the aggregateBy parameter.
		...
		for (int a = 1; a <= aggregateBy; a++) {
			...
			myPredecessors.add(predecessors.get(0)); //the precessor is now a predecessor of the new parallel
