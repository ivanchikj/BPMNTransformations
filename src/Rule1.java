...
//TODO la tua logica è sbagliata, non c'è bisogno che TUTTI i predecessori siano parallel gateway, perché tu non cancelli il gateway, ma solo i predecessori. Quindi basta che un parallel merge abbia un parallel come follower per poterlo cambiare. Devo vedere le cose dal punto di vista dell'outermost, non dell'innermost.
class Rule1 {

    static void applyRule(Model model) throws Exception {
//    System.out.println("I'm applying rule RULE1 to model " + startingModel.path);

        // Here I'm creating a list of all the parallel gateways in the inputModel
        NodeList parallelGatewayInstances = model.doc.getElementsByTagName(model.style("parallelGateway"));
        ...
        if (parallelGatewayInstances.getLength() == 0) {
            System.out.println("RULE1: there are no parallel gateways in this model");
            //RETURN FALSE
        }
...
//	    System.out.println("The id of the element is " + gateway.getAttribute("id") );
            ...
            if (model.isAMerge(gateway) && isApplicable(model, gateway)) {
                readyToBeChangedGateways.add(gateway); //I do the edits at the end to avoid recursive behavior
            }
            ...
            for (Element gateway : readyToBeChangedGateways) {
                ...
                for (Element pred : predecessors) {
                    ...
                    for (Element aFlowsToKeep : flowsToKeep) {
                        model.setTarget(aFlowsToKeep.getAttribute("id"), gateway.getAttribute("id"));
                    }
     ...
     *
     */
    private static boolean isApplicable(Model model, Element parallelGateway) throws XPathExpressionException {
        ...
        for (Element predecessor : predecessors) {
            ...
            ArrayList<Element> predsOfPred = model.getPredecessors(predecessor); //the predecessors of the predecessor
